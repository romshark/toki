...
var writers_ru = map[string]func(w io.Writer, args ...any) (int, error){
	...
	msgd2497314df5ae7e6: func(w io.Writer, args ...any) (written int, err error) {
		...
		written += n
		n, err = io.WriteString(w, tr_ru.FmtDateFull(args[0].(time.Time)))
		if err != nil {
			return written, err
		}
		written += n
		...
		written += n
		n, err = io.WriteString(w, tr_ru.FmtTimeFull(args[1].(time.Time)))
		if err != nil {
			return written, err
		}
		written += n
	...
	msgdc0a1830b671625c: func(w io.Writer, args ...any) (written int, err error) {
		...
		written += n
		switch pluralRuleCardinal(tr_ru, args[0]) {
		default:
		...
		written += n
		switch pluralRuleCardinal(tr_ru, args[1]) {
		default:
	...
	msgf5b4499f95971294: func(w io.Writer, args ...any) (written int, err error) {
		...
		written += n
		n, err = io.WriteString(w, tr_ru.FmtDateFull(args[1].(time.Time)))
		if err != nil {
			return written, err
		}
		written += n
		...
		written += n
		n, err = io.WriteString(w, tr_ru.FmtTimeFull(args[2].(time.Time)))
		if err != nil {
			return written, err
		}
		written += n
...
func (catalog_ru) Locale() language.Tag { return language.MustParse("ru") }

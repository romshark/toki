...
`toki edit` opens a desktop editor app for comfortable manual translation editing
(pass `-server <addr>` to run it as a plain HTTP server instead).
...
**TIP:** Use `toki edit` to open the native desktop editor for translations
(or `toki edit -server <addr>` to run it as a plain HTTP server).
